...
function updateEstimation() {
  ...
  let baseReward = 0;
  if (mode === 'practice') {
    baseReward = score * 0.01;
  } else if (mode === 'bot') {
    baseReward = score * 0.005;
  } else if (mode === 'ranked') {
    baseReward = bet * 1.8;
  }
  
  const total = baseReward + bonus;
  rewardEstimation.innerText = `${total.toFixed(2)} USDC`;
  estBase.innerText = baseReward.toFixed(2);
  estBonus.innerText = bonus.toFixed(2);
}
...
async function refreshServerStats() {
  try {
    ...
    srvUsdcBase.innerText = data.baseUsdcBalance.toFixed(2);
    srvUsdcPer.innerText = data.ephemeralUsdcBalance.toFixed(2);
...
async function refreshUserStats() {
  ...
  try {
    ...
    // 2. Fetch Base USDC Balance
    const mintPubkey = new solanaWeb3.PublicKey(USDC_DEVNET_MINT);
    ...
    try {
      ...
      userUsdcBase.innerText = tokenBal.value.uiAmountString;
    } catch (ataErr) {
      userUsdcBase.innerText = '0.00'; // account doesn't exist
    }
    
    // 3. Fetch Ephemeral USDC Balance
    if (userToken && Date.now() < userTokenExpiry) {
      userUsdcPer.innerText = 'Loading...';
      const pubkeyStr = userWallet.toBase58();
      
      const res = await fetch(`${MAGICBLOCK_API_URL}/v1/spl/private-balance?address=${pubkeyStr}&mint=${USDC_DEVNET_MINT}&cluster=devnet`, {
        headers: {
      ...
      userUsdcPer.innerText = (rawBal / 1_000_000).toFixed(2);
      log(`PER Private balance loaded: ${rawBal / 1_000_000} USDC`, 'success');
    } else {
...
btnDeposit.addEventListener('click', async () => {
  ...
  const baseUnits = Math.round(uiAmount * 1_000_000);
  
  try {
    log(`Requesting deposit transaction from Magicblock API for ${uiAmount} USDC (${baseUnits} base units)...`, 'info');
    ...
    log('USDC deposit (Shielding) confirmed successfully!', 'success');
...
// Unshield (Withdraw) USDC from PER
btnWithdraw.addEventListener('click', async () => {
  ...
  try {
    log(`Requesting withdrawal transaction for ${uiAmount} USDC (${baseUnits} base units)...`, 'info');
    ...
    log('USDC withdrawal (Unshielding) confirmed successfully!', 'success');
...
// Direct Bet USDC from User Base → Server PER (private transfer)
btnDirect.addEventListener('click', async () => {
  ...
  try {
    log(`🎮 Placing direct bet of ${uiAmount} USDC (Base → Server PER)...`, 'info');
    ...
    log(`✅ Direct Bet confirmed! ${uiAmount} USDC wagered from Base → Server PER.`, 'success');
...
// Send USDC from User PER → Server PER (private ephemeral-to-ephemeral)
btnSend.addEventListener('click', async () => {
  ...
  try {
    log(`🎮 Sending ${uiAmount} USDC from your PER → Server PER (private transfer)...`, 'info');
